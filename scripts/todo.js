...
tabSearch.addEventListener('click', () => {
  ...
  input.addEventListener('input', () =>
    renderTodos(todos.filter(t => t.title.toLowerCase().includes(input.value.toLowerCase())))
  );
...
tabFilter.addEventListener('click', () => {
  ...
  input.addEventListener('input', () =>
    renderTodos(todos.filter(t => t.tags.includes(input.value)))
  );
  div.appendChild(input);
  panelArea.appendChild(div);
});
...
function editTask(idx) {
  ...
  subDiv.classList.add('subtasks-panel');
  const subUl = document.createElement('ul');
  subUl.classList.add('subtasks');
  ...
  subInput.type = 'text';
  subInput.placeholder = 'Add subtask & Enter';
